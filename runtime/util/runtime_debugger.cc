...
bool IsKvCacheTensor(absl::string_view tensor_name) {
  return absl::StrContains(tensor_name, "kv_cache") ||
         absl::StrContains(tensor_name, "k_cache") ||
         absl::StrContains(tensor_name, "v_cache");
}
...
void RuntimeDebugger::CaptureTensors(
    ...
    const absl::flat_hash_map<absl::string_view, ::litert::TensorBuffer>&
        tensors) {
  SessionContext* session = cur_session_.load(std::memory_order_acquire);
  absl::string_view target_dir =
      session != nullptr ? session->capture_dir : capture_dir_;
  if (target_dir.empty()) {
    return;
  }

  bool is_prefill = IsPrefillSignature(signature_name);
  ...
  for (const auto& [name, buffer] : tensors) {
    if (!is_prefill && IsKvCacheTensor(name)) {
      // Skip redundant KV cache writes on intermediate decode steps.
      continue;
    }

    absl::Status status = SaveTensorBufferToFile(
...
void RuntimeDebugger::RegisterSession(int session_id,
                                      absl::string_view preferred_cache_dir) {
  ...
  std::string session_dir = (std::filesystem::path{std::string(base_dir)} /
...
void RuntimeDebugger::UnregisterSession(int session_id) {
  absl::MutexLock lock(mutex_);
  auto it = sessions_.find(session_id);
  if (it != sessions_.end()) {
    if (cur_session_.load(std::memory_order_relaxed) == it->second.get()) {
      cur_session_.store(nullptr, std::memory_order_release);
    }
    sessions_.erase(it);
  }
}

void RuntimeDebugger::SetActiveSession(int session_id) {
  absl::MutexLock lock(mutex_);
  auto it = sessions_.find(session_id);
  if (it != sessions_.end()) {
    cur_session_.store(it->second.get(), std::memory_order_release);
  } else {
    cur_session_.store(nullptr, std::memory_order_release);
  }
}
...
void RuntimeDebugger::ObserveTokens(int session_id,
  ...
  std::string target_dir = capture_dir_;
...
void RuntimeDebugger::WriteTokensToFile(absl::string_view target_dir,
  ...
  std::string capture_path = (std::filesystem::path{std::string(target_dir)} /
