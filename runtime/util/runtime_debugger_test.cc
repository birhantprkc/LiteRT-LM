...
TEST(RuntimeDebuggerTest, SessionLifecycleAndSwitching) {
  ...
  debugger->RegisterSession(/*session_id=*/0);
  debugger->RegisterSession(/*session_id=*/1);

  // Switch to session 0 and verify tokens are captured to session 0 dir.
  debugger->SetActiveSession(/*session_id=*/0);
  ...
  debugger->SetActiveSession(/*session_id=*/999);

  // Unregister session 0 and verify cleanup.
  debugger->SetActiveSession(/*session_id=*/0);
  debugger->UnregisterSession(/*session_id=*/0);
}
